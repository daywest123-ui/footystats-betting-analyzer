...
Uses openfootball/football.json (public-domain/CC0) for fixtures/results and
football-data.co.uk downloadable CSVs for published pre-match odds.
...
FOOTBALL_DATA={
 ...
 "England League Two":"E3","Germany Bundesliga":"D1","Germany 2. Bundesliga":"D2",
 "Spain La Liga":"SP1","Italy Serie A":"I1","France Ligue 1":"F1",
 "Netherlands Eredivisie":"N1","Portugal Primeira Liga":"P1","Greece Super League":"G1",
 "Turkey Super Lig":"T1"}
    ...
    for league,file in OPENFOOTBALL.items():
        try:
            ...
            for m in data.get("matches",[]):
                h,a=m.get("team1"),m.get("team2"); d=_parse_date(m.get("date",""))
                if not h or not a or not d: continue
                ...
                out.append({"home":h.strip(),"away":a.strip(),"league":league,"date":d,
...
def recent_form(matches:list[dict[str,Any]],team:str,before:str,limit:int=8)->dict[str,Any]:
    key=_norm(team)
    rows=[m for m in matches if m["finished"] and m["date"]<before and
          (_norm(m["home"])==key or _norm(m["away"])==key)]
    ...
    for m in rows:
        hg,ag=m["home_goals"],m["away_goals"]; home=_norm(m["home"])==key
        gf,ga=(hg,ag) if home else (ag,hg)
        pts+=3 if gf>ga else 1 if gf==ga else 0; gd+=gf-ga
        over+=int(hg+ag>=3); btts+=int(hg>0 and ag>0)
    n=len(rows)
    return {"matches":n,"points_per_game":pts/n,"goal_diff_per_game":gd/n,
...
def fixture_odds(home:str,away:str,day:str)->dict[str,float]:
    ...
    cols={"home_win":("B365H","BWH","IWH","PSH","WHH"),
    ...
    for league in FOOTBALL_DATA:
        for row in _csv_rows(league):
            if _parse_date(row.get("Date",""))!=day: continue
            if _norm(row.get("HomeTeam",""))!=hk or _norm(row.get("AwayTeam",""))!=ak: continue

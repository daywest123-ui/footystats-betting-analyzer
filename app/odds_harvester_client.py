...
TIMEOUT_SECONDS = int(os.environ.get("ODDSHARVESTER_TIMEOUT_SECONDS", "90"))
...
def _run_cli(day: str, markets: str = "1x2,btts,over_under") -> list[dict[str, Any]] | None:
    output = Path(tempfile.gettempdir()) / f"footy_oddsharvester_{day}_{markets.replace(',', '_')}.json"
    if output.exists():
        output.unlink()
    cli_day = day.replace("-", "")
    cmd = [
        "oddsharvester", "upcoming", "-s", "football", "-d", cli_day,
        "-m", markets,
    ...
    try:
        ...
        try:
            ...
            return None
        print(f"[OddsHarvester] Retrieved {len(records)} records for {day} ({markets})")
        return records
    except subprocess.TimeoutExpired:
        print(f"[OddsHarvester] Timeout after {TIMEOUT_SECONDS}s for {day} ({markets})")
        return None
...
def collect_day(day: str) -> list[dict[str, Any]]:
    ...
    # OddsPortal can fail while initializing one market selector even though
    # the basic 1X2 market remains scrapeable. Try the full request once, then
    # degrade to 1X2 rather than launching another scraper per fixture.
    records = _run_cli(day, "1x2,btts,over_under")
    if records is None:
        print(f"[OddsHarvester] Retrying {day} with 1x2-only fallback")
        records = _run_cli(day, "1x2")
    _CACHE[day] = records or []
    return _CACHE[day]

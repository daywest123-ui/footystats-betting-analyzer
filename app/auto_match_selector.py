...
from football_data_client import load_openfootball, recent_form as open_recent_form, fixture_odds as open_fixture_odds
...
_OPEN_MATCHES: list[dict] = []

def discover_fixtures(date: datetime) -> list[dict]:
    """Discover today's fixtures from openfootball public-domain JSON.
    No API key, provider token, or paid feed is used.
    """
    ...
    for m in _OPEN_MATCHES:
        ...
        fixtures.append({
            ...
            "fixture_date": m["date"], "fixture_id": f"{m['home']}||{m['away']}||{m['date']}",
...
def _recent_form(team_name: str | None, before: str | None = None) -> dict:
    if not team_name:
        return _empty_form()
    before = before or datetime.now(timezone.utc).date().isoformat()
    if _OPEN_MATCHES:
        return open_recent_form(_OPEN_MATCHES, team_name, before, 8)
    return _empty_form()
...
def score_fixture(fixture: dict) -> dict:
    home_form = _recent_form(fixture.get("home"), fixture.get("fixture_date"))
    away_form = _recent_form(fixture.get("away"), fixture.get("fixture_date"))
...
def main() -> None:
    ...
    source_degraded = not results or any(r.get("data_quality", {}).get("min_recent_matches", 0) < 3 for r in results)
    report = {
        ...
        "data_sources": ["openfootball/football.json (public domain)", "football-data.co.uk CSV (free downloadable odds)", "public web"],
